...
if((count($_POST) > 2)&&(!($_POST['Region'] == "not set" ))){
	...
	if($_POST['total_for_text'] == 0.00){
		echo json_encode(array("Image" => "http://dev.triplecrowncustom.com/dev/wp-content/themes/triple/img/default.png", "Region" => $_POST['Region']));
		unset($_POST);
	}
	...
	$linkToFile = "http://dev.triplecrowncustom.com/app/embroidery_files/".$filename."_".$token.".png";
...
switch($newRegion){
	...
	// session file path information
	$_SESSION['neckFilePath'] = $fakePath;

	echo json_encode(array("Image" => $linkToFile, "Region" => $newRegion));
	...
	// session file path information
	$_SESSION['centerFilePath'] = $fakePath;
	...
	// session file path information
	$_SESSION['shoulderFilePath'] = $fakePath;
	...
	echo json_encode(array("Error" => "Some how not set", "Posted Data"=>$_POST));

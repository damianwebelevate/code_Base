...
<div class="breadTwoContainer">
	...
	<h1 class="title"><span></span>Payment Details</h1>
		...
		<span class='payment-errors'></span>
			...
			<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
				<fieldset class="form-group">
					<span class="loading"><img src="http://dev.triplecrowncustom.com/dev/wp-content/themes/triple/img/ajax-loader.gif"> <span class="loadingText">Please wait...</span></span>
...
$j( document ).ready(function() {
	...
	$j("#customer_shipping_address_state, #customer_billing_address_state").on('change', function(evt) {
		...
		if (state && value) {
			$j.ajax({
				url: 'http://dev.triplecrowncustom.com/dev/wp-content/themes/triple/site/shipping_fees.php',
	...
	$j('#customer_cc_fullname').blur(function(evt){
		var val = $j(this).val();
		var sub = "'";
	...
	$j('#customer_cc_exp_year').blur(function(evt){
		var date = new Date();
		...
		if (response.error) {
			...
			if ($j("#customer_confirm_payment").is(':checked')) {
				...
				if (errorsFound > 0) {
					...
					$j.ajax({
						url: 'http://dev.triplecrowncustom.com/dev/wp-content/themes/triple/site/step_4.php',
						data: data,
						type: 'POST',
						dataType: 'json',
						success: function(json) {
